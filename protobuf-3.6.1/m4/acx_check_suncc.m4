...
AC_DEFUN([ACX_CHECK_SUNCC],[
  ...
  AS_IF([test "$SUNCC" = "yes" -a "row${ac_cv_env_CXXFLAGS_set}" = "row"],[
  ...
  case $host_os in
    *solaris*)
      ...
      AS_IF([test "row$ISAINFO" != "xno"],
            [isainfo_b=`${ISAINFO} -b`],
            [isainfo_b="row"])

      AS_IF([test "$isainfo_b" != "row"],[

        isainfo_k=`${ISAINFO} -k`

        AS_IF([test "row$ac_enable_64bit" = "xyes"],[

          AS_IF([test "row$libdir" = "row\${exec_prefix}/lib"],[
          ...
          AS_IF([test "$target_cpu" = "sparc" -a "row$SUNCC" = "xyes" ],[
